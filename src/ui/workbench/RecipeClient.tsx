...
export function RecipeClient({ dossier, studies, locale }: { dossier: Dossier; studies: Study[]; locale: Locale }) {
  ...
  return <main id="main-content" className="container workbench" tabIndex={-1}>
    <WorkbenchTopline locale={locale} section={tx(locale,'Sürü reçeteleri','Swarm recipes')}><div className="wb-actions"><button className="action action-outlined" type="button" onClick={()=>download('markdown')}><Icon name="download" />{tx(locale,'Markdown indir','Download Markdown')}</button><button className="action action-outlined" type="button" onClick={()=>download('json')}><Icon name="download" />{tx(locale,'JSON indir','Download JSON')}</button></div></WorkbenchTopline>
    ...
    <div className="wb-article-layout"><article className="wb-article-body"><figure className="wb-mechanism-band"><MechanismDiagram topology={p.topology} locale={locale} /><figcaption>{tx(locale,'İletişim mantığının kavramsal şeması · agent sayısını görev paketinde belirle','Conceptual communication diagram · set the agent count in the task package')}</figcaption></figure>
      <div className="wb-tabs" role="group" aria-label={tx(locale,'Reçete bölümü','Recipe section')}>{([['protocol',tx(locale,'Protokol','Protocol')],['experiment',tx(locale,'Deney planı','Experiment plan')],['sources',tx(locale,'Kaynaklar','Sources')]] as const).map(([key,label])=><button key={key} type="button" aria-pressed={tab===key} onClick={()=>update({tab:key})}>{label}</button>)}</div>
      ...
      <label>{tx(locale,'Görevin','Your task')}<textarea value={config.task} onChange={event=>changeConfig({task:event.target.value})} required minLength={10} maxLength={6000} rows={5}/></label>
      <label>{tx(locale,'Agent sayısı','Agent count')}<input type="number" min="3" max="32" step="1" value={Number.isFinite(config.agents)?config.agents:''} onChange={event=>changeConfig({agents:event.target.valueAsNumber})} required /></label>
      <label>{tx(locale,'Tur sınırı','Round limit')}<input type="number" min="1" max="12" step="1" value={Number.isFinite(config.rounds)?config.rounds:''} onChange={event=>changeConfig({rounds:event.target.valueAsNumber})} required /></label>
      <label>{tx(locale,'Toplam token bütçesi','Total token budget')}<input type="number" min="1000" max="1000000" step="1" value={Number.isFinite(config.tokenBudget)?config.tokenBudget:''} onChange={event=>changeConfig({tokenBudget:event.target.valueAsNumber})} required /><small>{tx(locale,'Bütçe tüm sürü içindir; planlama ve doğrulama dahildir.','Budget covers the entire swarm, including planning and verification.')}</small></label>
      <button type="submit" className="action action-primary">{tx(locale,'Görev paketini hazırla','Prepare task package')}</button>
    ...
    {prepared&&<section ref={output} className="wb-package" tabIndex={-1} style={{marginTop:36,scrollMarginTop:20}} aria-labelledby="package-title"><div className="wb-section-heading"><h2 id="package-title">{tx(locale,'Görev paketin','Your task package')}</h2><div className="wb-filter-group" role="group" aria-label={tx(locale,'Paket biçimi','Package format')}><button type="button" aria-pressed={previewFormat==='markdown'} onClick={()=>setPreviewFormat('markdown')}>Markdown</button><button type="button" aria-pressed={previewFormat==='json'} onClick={()=>setPreviewFormat('json')}>JSON</button></div></div><pre className="wb-code"><code>{previewFormat==='markdown'?recipeMarkdown(prepared):JSON.stringify(prepared,null,2)}</code></pre><div className="wb-actions"><button className="action action-primary" type="button" onClick={()=>download(previewFormat)}><Icon name="download" />{tx(locale,'Paketi indir','Download package')}</button><button className="action action-outlined" type="button" onClick={toAgenda}><Icon name="plus" />{tx(locale,'Deney planını gündeme ekle','Add experiment plan to agenda')}</button></div></section>}
  </main>
}
