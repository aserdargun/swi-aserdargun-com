...
import { LaboratoryCollection } from './Laboratory'
  ...
  return <main id="main-content" className="container workbench" tabIndex={-1}>
    ...
    <section className="wb-home-hero" aria-labelledby="workspace-title">
      <div className="wb-hero-copy"><h1 id="workspace-title">{tx(locale,'Kolektif zekâyı kendi sürüne taşı.','Bring collective intelligence to your swarm.')}</h1>
        <p>{tx(locale,'Doğadaki mekanizmaları incele. Araştırmayı sorgula. Kendi ortamında sınamak için kaynaklı bir deney planı hazırla.','Study mechanisms in nature. Question the research. Prepare a sourced experiment plan to test in your own environment.')}</p>
    ...
    <div className="wb-home-body"><section className="wb-featured" aria-labelledby="from-nature"><div className="wb-section-heading"><h2 id="from-nature">{tx(locale,'Doğadan al, süründe dene','From nature to your swarm')}</h2><TextLink href={path('/atlas/')}>{tx(locale,`${dossiers.length} dosya`,`${dossiers.length} dossiers`)}</TextLink></div>
